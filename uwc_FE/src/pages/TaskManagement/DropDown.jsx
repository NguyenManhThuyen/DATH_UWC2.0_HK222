...
const DropdownHook = ({control, setValue, name}) => {
    ...
    return (
        <div className='relative' ref={nodeRef}>
            <div className='px-3 py-2 rounded-lg border border-gray-100 bg-[#D9D9D9] flex items-center justify-between cursor-pointer' onClick={() => setShow(!show)}>
                <span>{label}</span>
            </div>
            <div className={`absolute top-full left-0 w-full rounded-lg bg-[#D9D9D9] ${show ? "" : "opacity-0 invisible"}`}>
                {
                    dropdownData.map((item) => (
                        <div className='p-4  cursor-pointer transition-all hover:bg-white' 
